...
import { LightingDocumentViewer } from './LightingDocumentViewer';
...
export function GeneratedDocumentsPanel({ measurement, onGenerated }: Props) {
  ...
  return <section className="mt-4 rounded-2xl border border-indigo-200 dark:border-indigo-900 bg-white dark:bg-slate-900 p-5 space-y-4">
    ...
    {loading ? <div className="text-sm text-slate-500">Cargando documentos...</div> : documents.length ? <div className="space-y-2">{documents.map((document) => <div key={document.id} className="rounded-xl bg-slate-50 dark:bg-slate-800 p-3 text-sm"><div className="flex justify-between gap-3"><div><div className="font-bold">{document.templateKey} <span className="text-slate-500 font-normal">v{document.templateVersion}</span></div><div className="text-xs text-slate-500 mt-1">Generado: {document.generatedAt ? new Date(document.generatedAt).toLocaleString() : '—'} · Estado: {document.status}</div><div className="text-xs text-slate-500 mt-1">ID: {document.id}</div></div>{document.protocolType === 'lighting' && <button type="button" onClick={() => setViewingDocumentId(document.id)} className="flex items-center gap-1 h-fit px-3 py-2 rounded-lg border text-xs font-bold"><Eye className="w-4 h-4"/>Ver</button>}</div></div>)}</div> : <div className="text-sm text-slate-500">Todavía no hay documentos generados para esta medición.</div>}
    {viewingDocumentId && <LightingDocumentViewer documentId={viewingDocumentId} onClose={() => setViewingDocumentId(null)} />}
