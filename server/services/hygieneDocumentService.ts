...
export async function createGeneratedDocument(input: { measurement: hygieneService.HygieneMeasurementRecord; generatedBy: string; templateKey: string; templateVersion: string; }): Promise<HygieneGeneratedDocument> {
  ...
  const now = new Date().toISOString(); const instrumentSnapshots: hygieneService.HygieneInstrumentRecord[] = [];
  for (const instrumentId of input.measurement.instrumentIds) { const instrument = await hygieneService.getInstrumentById(instrumentId, input.measurement.orgId); if (!instrument) throw new Error(`INSTRUMENT_NOT_FOUND:${instrumentId}`); instrumentSnapshots.push(JSON.parse(JSON.stringify(instrument))); }
  const document: HygieneGeneratedDocument = { id: newId(), orgId: input.measurement.orgId, measurementId: input.measurement.id, protocolType: input.measurement.protocolType, templateKey: input.templateKey, templateVersion: input.templateVersion, status: "generated", generatedBy: input.generatedBy, generatedAt: now, measurementSnapshot: { id: input.measurement.id, context: { ...input.measurement.context }, protocolType: input.measurement.protocolType, measurementDate: input.measurement.measurementDate, instrumentIds: [...input.measurement.instrumentIds], instrumentSnapshots, rawData: input.measurement.rawData ? JSON.parse(JSON.stringify(input.measurement.rawData)) : undefined, notes: input.measurement.notes ?? null, normativeEvaluationSnapshot: input.measurement.normativeEvaluationSnapshot ? JSON.parse(JSON.stringify(input.measurement.normativeEvaluationSnapshot)) : undefined, review: input.measurement.review ? { ...input.measurement.review } : undefined, status: input.measurement.status, validatedAt: input.measurement.review?.reviewedAt } };
...
export function buildLightingDocumentRepresentation(document: HygieneGeneratedDocument): HygieneDocumentRepresentation {
  ...
  const regulatoryReference = (normative.reference ?? {}) as Record<string, unknown>;
  if (!regulatoryReference.id) throw new Error("REGULATORY_REFERENCE_SNAPSHOT_REQUIRED");
  if (LIGHTING_DOCUMENT_TEMPLATE.regulatoryReferenceId !== regulatoryReference.id) throw new Error("REGULATORY_REFERENCE_TEMPLATE_MISMATCH");
  const context = snapshot.context as unknown as Record<string, unknown>; const raw = snapshot.rawData ?? {}; const lighting = (raw.lighting ?? raw) as Record<string, unknown>; const points = Array.isArray(lighting.points) ? lighting.points : []; const review = snapshot.review as unknown as Record<string, unknown> | undefined;
  const sections = [
