...
export const hygieneService = {
  ...
  async saveNormativeSnapshot(id: string, normativeProtocolVersionId: string, criterionId?: string): Promise<HygieneMeasurement> { const { measurement } = await request<{ measurement: HygieneMeasurement }>(`/measurements/${encodeURIComponent(id)}/normative-snapshot`, { method: 'POST', body: JSON.stringify({ normativeProtocolVersionId, criterionId }) }); return measurement; },
  ...
  async getLightingNormativeVersions(): Promise<NormativeProtocolVersionRecord[]> { const { normativeProtocolVersions } = await request<{ normativeProtocolVersions: NormativeProtocolVersionRecord[] }>('/normative/protocols?protocolType=lighting&status=active'); return normativeProtocolVersions; },
};
