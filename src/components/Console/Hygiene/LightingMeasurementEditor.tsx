...
import { HygieneMeasurement, LightingCampaignData, LightingMeasurementData, LightingMeasurementPoint } from '../../../types/safety';
...
export const LightingMeasurementEditor: React.FC<Props> = ({ measurement, onSaved }) => {
  ...
  const [campaign, setCampaign] = useState<LightingCampaignData>(existing?.campaign || {});
  ...
  const updateCampaign = (field: keyof LightingCampaignData, value: string) => {
    setCampaign((current) => ({ ...current, [field]: value || undefined }));
  };
  ...
  const preview = useMemo(() => {
    try {
      return calculateLightingMeasurement({ sourceType, lightingSystem, taskDescription, campaign, points: points.filter((point) => point.name.trim() && Number(point.lux) >= 0) });
    } catch { return null; }
  }, [sourceType, lightingSystem, taskDescription, campaign, points]);
  ...
  const save = async () => {
    ...
    try {
      ...
      const data = calculateLightingMeasurement({ sourceType, lightingSystem, taskDescription, campaign, points: validPoints });
  ...
  return <><div className="mt-4 rounded-2xl border border-amber-200 dark:border-amber-900 bg-white dark:bg-slate-900 p-5 space-y-5">
    <div><h3 className="font-extrabold text-slate-900 dark:text-white">Editor de Iluminación</h3><p className="text-sm text-slate-500 mt-1">Carga de datos técnicos, contexto de campaña y cálculo de indicadores descriptivos.</p></div>

    <div className="rounded-2xl border border-slate-200 dark:border-slate-800 p-4 space-y-3">
      <h4 className="font-extrabold">Datos de la campaña</h4>
      <div className="grid md:grid-cols-2 gap-3">
        <label className="text-sm font-medium">Hora de inicio<input type="time" value={campaign.startTime || ''} onChange={(e) => updateCampaign('startTime', e.target.value)} className="mt-1 w-full rounded-xl border p-2.5 bg-transparent"/></label>
        <label className="text-sm font-medium">Hora de finalización<input type="time" value={campaign.endTime || ''} onChange={(e) => updateCampaign('endTime', e.target.value)} className="mt-1 w-full rounded-xl border p-2.5 bg-transparent"/></label>
        <label className="text-sm font-medium md:col-span-2">Metodología<textarea value={campaign.methodology || ''} onChange={(e) => updateCampaign('methodology', e.target.value)} rows={2} placeholder="Describir método de medición aplicado" className="mt-1 w-full rounded-xl border p-2.5 bg-transparent"/></label>
        <label className="text-sm font-medium">Condiciones atmosféricas<textarea value={campaign.atmosphericConditions || ''} onChange={(e) => updateCampaign('atmosphericConditions', e.target.value)} rows={2} className="mt-1 w-full rounded-xl border p-2.5 bg-transparent"/></label>
        <label className="text-sm font-medium">Condiciones habituales del puesto<textarea value={campaign.workplaceConditions || ''} onChange={(e) => updateCampaign('workplaceConditions', e.target.value)} rows={2} className="mt-1 w-full rounded-xl border p-2.5 bg-transparent"/></label>
        <label className="text-sm font-medium">URL certificado de calibración<input type="url" value={campaign.calibrationCertificateUrl || ''} onChange={(e) => updateCampaign('calibrationCertificateUrl', e.target.value)} placeholder="https://..." className="mt-1 w-full rounded-xl border p-2.5 bg-transparent"/></label>
        <label className="text-sm font-medium">URL plano / croquis<input type="url" value={campaign.planOrSketchUrl || ''} onChange={(e) => updateCampaign('planOrSketchUrl', e.target.value)} placeholder="https://..." className="mt-1 w-full rounded-xl border p-2.5 bg-transparent"/></label>
        <label className="text-sm font-medium md:col-span-2">Observaciones de campaña<textarea value={campaign.observations || ''} onChange={(e) => updateCampaign('observations', e.target.value)} rows={2} className="mt-1 w-full rounded-xl border p-2.5 bg-transparent"/></label>
      </div>
    </div>
    ...
    <div><div className="flex items-center justify-between mb-3"><h4 className="font-bold">Puntos de medición</h4><button type="button" onClick={() => setPoints((current) => [...current, emptyPoint()])} className="flex items-center gap-1 text-sm font-bold text-indigo-600"><Plus className="w-4 h-4"/> Agregar punto</button></div>
      <div className="space-y-3">{points.map((point, index) => <div key={index} className="grid grid-cols-1 md:grid-cols-4 gap-3 rounded-xl bg-slate-50 dark:bg-slate-800 p-3">
        <input value={point.name} onChange={(e) => updatePoint(index, 'name', e.target.value)} placeholder="Sector / identificación" className="rounded-lg border p-2 bg-transparent"/>
        ...
        <input type="time" value={point.measuredAt || ''} onChange={(e) => updatePoint(index, 'measuredAt', e.target.value)} className="rounded-lg border p-2 bg-transparent"/>
        <input value={point.sector || ''} onChange={(e) => updatePoint(index, 'sector', e.target.value)} placeholder="Sector" className="rounded-lg border p-2 bg-transparent"/>
        <input value={point.workplace || ''} onChange={(e) => updatePoint(index, 'workplace', e.target.value)} placeholder="Puesto" className="rounded-lg border p-2 bg-transparent"/>
        <select value={point.lightSourceType || ''} onChange={(e) => updatePoint(index, 'lightSourceType', e.target.value)} className="rounded-lg border p-2 bg-transparent"><option value="">Fuente</option><option value="natural">Natural</option><option value="artificial">Artificial</option><option value="mixed">Mixta</option></select>
        <input value={point.observations || ''} onChange={(e) => updatePoint(index, 'observations', e.target.value)} placeholder="Observaciones" className="rounded-lg border p-2 bg-transparent md:col-span-3"/>
    ...
    {showEvaluation && preview && measurement.normativeEvaluationSnapshot && (() => {
      ...
      return <div className="rounded-2xl border border-indigo-200 dark:border-indigo-900 p-4 space-y-3"><div><h4 className="font-extrabold">Evaluación asistida</h4><p className="text-xs text-slate-500">{evaluation.normativeReference} · versión {evaluation.normativeVersion}</p></div><div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-sm">{[['Promedio', evaluation.summary.averageLux],['Mínimo', evaluation.summary.minimumLux],['Máximo', evaluation.summary.maximumLux],['Puntos', evaluation.summary.pointsMeasured]].map(([label, value]) => <div key={String(label)} className="rounded-lg bg-slate-50 dark:bg-slate-800 p-2"><div className="text-xs text-slate-500">{label}</div><div className="font-bold">{value}</div></div>)}</div><div className="space-y-2">{evaluation.criteria.map((criterion) => <div key={criterion.criterionId} className="text-sm rounded-lg bg-slate-50 dark:bg-slate-800 p-3"><div className="font-bold">{criterion.code} — {criterion.title}</div>{criterion.observations.map((item, i) => <div key={i} className="text-slate-600 dark:text-slate-300 mt-1">• {item}</div>)}</div>)}</div><div className="rounded-lg bg-amber-50 dark:bg-amber-950/30 p-3 text-sm">Resultado: requiere revisión profesional. La aplicación no emite una declaración automática de cumplimiento.</div></div>;
    })()}
