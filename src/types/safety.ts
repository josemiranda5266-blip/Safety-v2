...
export interface DocComparisonResult {
  ...
  similarities: string[];
  differences: string[];
  normativeDetails: { docTitle: string; position: string; requirements: string }[];
  date: string;
}
...
export interface LightingMeasurementData { sourceType: LightingSourceType; lightingSystem?: string; taskDescription?: string; points: LightingMeasurementPoint[]; averageLux?: number; minimumLux?: number; maximumLux?: number; uniformityRatio?: number; calculationVersion: string; calculatedAt?: string; }
